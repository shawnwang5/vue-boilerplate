export class DateUtils {
  static getCountdownStr(timeStr: string) {
    ...
    if (time - nowTime > 0) {
      ...
      let hour =
        (timeDiff - Math.floor(day) * TOTAL_TIME_FOR_DAY) / TOTAL_TIME_FOR_HOUR;
      let minute =
        (timeDiff -
          Math.floor(day) * TOTAL_TIME_FOR_DAY -
          Math.floor(hour) * TOTAL_TIME_FOR_HOUR) /
        TOTAL_TIME_FOR_MINUTE;
      let second =
        (timeDiff -
          Math.floor(day) * TOTAL_TIME_FOR_DAY -
          Math.floor(hour) * TOTAL_TIME_FOR_HOUR -
          Math.floor(minute) * TOTAL_TIME_FOR_MINUTE) /
        TOTAL_TIME_FOR_SECOND;
