...
export class FileUtils {
  ...
  static base64ToFile(b64Data: string, extension = "png") {
    b64Data = b64Data.replace(
      FileUtils.imageBase64HeaderMap.get(extension) || "",
      ""
    );
    const sliceSize = 512;
    ...
    return new File(byteArrays, `image.${extension}`, {
      type: FileUtils.imageContentTypeMap.get(extension),
    });
  }
  ...
  static isWordDoc(url: string): boolean {
    return /\.docx?/.test(url);
  }

  static isExcel(url: string): boolean {
    return /\.xlsx?/.test(url);
  }

  static openFile(url: string): void {
    window.open(url);
  }
}
