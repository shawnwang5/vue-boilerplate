<template>
  <div class="radio" :class="[checked ? 'selected' : '', radioStyleType]">
    ...
    />
    <label :for="radioId"
      ><i class="icon" :class="[radioStyleType]"></i>{{ radioLabel }}</label
    >
  </div>
</template>
...
.radio {
  ...
  &.circle {
    label > .icon.circle {
      ...
      vertical-align: top;
      margin-right: 0.2rem;
      background: url("../../assets/images/common-icon/icon_radio.png") center /
        100% 100% no-repeat;
  ...
  input[type="radio"] {
    ...
    &:checked + label {
      ...
      & > .icon.circle {
        background: transparent
          url("../../assets/images/common-icon/icon_radio_checked.png") center /
          100% 100% no-repeat;
        border: none;
